...
const COMPACT_COMBINATIONS: string[] = [
...
const COLUMNS: string[] = ['1A', '1B', '1D', '1E', '1G', '1I', '1K', '1L'];

// Expande una combinacion compacta a objeto completo
function expandCombination(compactStr: string, optionNum: number): ThirdPlaceCombination {
...
let _expandedCache: ThirdPlaceCombination[] | null = null;

// Obtener todas las combinaciones (expandidas bajo demanda)
export function getThirdPlaceCombinations(): ThirdPlaceCombination[] {
  if (!_expandedCache) {
...
export const thirdPlaceCombinations = {
  get length(): number { return COMPACT_COMBINATIONS.length; },
  [Symbol.iterator]: function* (): Generator<ThirdPlaceCombination> { yield* getThirdPlaceCombinations(); },
  find: (fn: (combo: ThirdPlaceCombination) => boolean): ThirdPlaceCombination | undefined =>
    getThirdPlaceCombinations().find(fn),
  filter: (fn: (combo: ThirdPlaceCombination) => boolean): ThirdPlaceCombination[] =>
    getThirdPlaceCombinations().filter(fn),
  map: <T>(fn: (combo: ThirdPlaceCombination) => T): T[] =>
    getThirdPlaceCombinations().map(fn),
};

// Funcion para obtener la combinacion correcta basada en los grupos que clasificaron terceros
export function getThirdPlaceCombination(qualifyingThirdPlaceGroups: string[]): ThirdPlaceCombination | undefined {
...
export function determineBestThirdPlaces(thirdPlaceTeams: ThirdPlaceTeam[]): ThirdPlaceTeam[] {
...
export const thirdPlaceMatchMapping: Record<string, string> = {
  'M74': '1E', 'M77': '1I', 'M79': '1A', 'M80': '1L',
  'M81': '1D', 'M82': '1G', 'M85': '1B', 'M87': '1K',
};

// Pools de grupos para cada partido
export const thirdPlacePools: Record<string, string[]> = {
  'M74': ['A', 'B', 'C', 'D', 'F'],
  'M77': ['C', 'D', 'F', 'G', 'H'],
  'M79': ['C', 'E', 'F', 'H', 'I'],
  'M80': ['E', 'H', 'I', 'J', 'K'],
...
export function getThirdPlaceAssignments(qualifyingGroups: string[]): ThirdPlaceAssignments | null {
