...
import { useState, useMemo, DragEvent, TouchEvent } from 'react';
...
interface TiebreakerModalProps {
  tie: TieData | null;
  onResolve: (group: string, order: number[]) => void;
  onClose: () => void;
}

export default function TiebreakerModal({ tie, onResolve, onClose }: TiebreakerModalProps): JSX.Element | null {
  ...
  // Handle drag start
  const handleDragStart = (e: DragEvent<HTMLDivElement>, teamId: number): void => {
    setDraggedItem(teamId);
    e.dataTransfer.effectAllowed = 'move';
    e.dataTransfer.setData('text/plain', teamId.toString());
  };
  ...
  const [touchedItem, setTouchedItem] = useState<{ teamId: number; index: number } | null>(null);

  const handleTouchStart = (e: TouchEvent<HTMLDivElement>, teamId: number, index: number): void => {
  ...
  // Get team data by ID
  const getTeamById = (teamId: number): TiedTeam | undefined => {
              ...
              return (
                ...
                >
                  ...
                  <div className="ml-auto text-xs text-muted-foreground flex-shrink-0">
                    {team.points} pts | DG: {(team.goalDifference ?? 0) > 0 ? '+' : ''}{team.goalDifference}
                  </div>
                </div>
              );
            })}
