...
import { mockTeams, getAllGroups, getTeamsByGroup, type MockTeam } from '@/data/mockData';
...
import { calculateGroupStandings, type Team, type MatchScore, type TeamStats, type UnresolvableTie, type GroupStandingsResult } from '@/utils/fifaTiebreaker';
import GroupStandingsTable from '@/components/GroupStandingsTable';
import TiebreakerModal from '@/components/TiebreakerModal';
import { Shield, Users, Trophy, ListOrdered, Check, X, RotateCcw, Save, AlertTriangle, LucideIcon } from 'lucide-react';
...
interface RealPlayoffResult {
  playoff_id: string;
  winner_team_id: number;
}

interface RealGroupMatch {
  group_letter: string;
  match_index: number;
  team_a_id: number;
  team_b_id: number;
...
interface TiebreakerDecision {
  tiedTeamIds: number[];
  resolvedOrder: number[];
}

interface TiebreakerDecisions {
  [groupLetter: string]: TiebreakerDecision;
}

interface CurrentTiebreaker extends UnresolvableTie {
  group: string;
}
...
interface LocalKnockoutResults {
  [matchKey: string]: LocalKnockoutResult;
}

interface ExtendedMockTeam extends MockTeam {
  actualTeamId?: number;
  thirdPlaceFrom?: string;
  isPlayoffWinner?: boolean;
}

interface AdminBracketMatchData {
  matchId: string;
  matchNumber: number;
...
function StatsTab({ stats }: StatsTabProps): JSX.Element {
  return (
...
function PlayoffsTab({ realPlayoffs, onSave, showSuccess, setError }: PlayoffsTabProps): JSX.Element {
...
function GroupsTab({ realPlayoffs, realGroupMatches, showSuccess, setError }: GroupsTabProps): JSX.Element {
  ...
  const openTiebreakerModal = useCallback((groupLetter: string, tie: UnresolvableTie): void => {
...
function KnockoutTab({ realPlayoffs, realGroupStandings, realKnockout, onSave, showSuccess, setError }: KnockoutTabProps): JSX.Element {
...
// ============================================
interface AdminBracketMatchProps {
...
function AdminBracketMatch({ match, onScoreChange, onSelectWinner, matchWidth }: AdminBracketMatchProps): JSX.Element | null {
...
function AdminBracket({ r32Matches, r16Matches, qfMatches, sfMatches, final, thirdPlace, onScoreChange, onSelectWinner, getTeamById }: AdminBracketProps): JSX.Element {
